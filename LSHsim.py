...
def main():
    ...
    if args.real_eval:
        ...
        for i, data in tqdm(enumerate(testloader)):
            ...
            for d in args.lshdim:
                if args.ideallshsim:
                    ...
                    value = metric_hamming_value(train_input, test_input, train_label, test_label, dim_plane=d, update=args.update, sum_argmax=args.sum_argmax)
                    accs.append(acc)
                    values.append(value)
                if args.crossbarsim:
            ...
            tem_value = np.array(Values_all)
            print(tem_value.shape)
        ...
        print('Test Point 1: ',np.shape(Values_all))
        print('Test Point 2: ',np.shape(Acc_all))
        np.savez('ACC.npz',Acc_all)
        np.savez('HD.npz', Values_all)
        ...
        np.savez(f'./results/LSH_inferencepyth/LSHomni_ideal_{args.eval_way}-way_{args.eval_shot}-shot_{args.eval_episode}-'
...
def metric_hamming(train_input, test_input, train_label, test_label, dim_plane=1024, update=False, sum_argmax=True):
...
def metric_hamming_value(train_input, test_input, train_label, test_label, dim_plane=1024, update=False, sum_argmax=True):
    hd_sum=0
    h_plane = np.random.randn(train_input.shape[-1], dim_plane)
    train_hashcode = ((train_input @ h_plane) > 0).astype(int)
...
if __name__ == '__main__':
    args = parser.parse_args('-c ./configs/lshconfig.config')
